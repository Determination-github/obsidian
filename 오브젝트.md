...
- 책임 중심 관점
	...
	- 객체의 행동에 초점을 맞춘다.
	- 객체를 협력하는 공동체 일원으로 본다.
	- 객체의 책임은 인터페이스에 속한다.


'Movie' 클래스 데이터 중심 설계
- 데이터 중심으로 우선 클래스를 만들어본다
- 데이터 중심 설계는 객체가 내부에 저장해야 하는 데이터가 무엇인가로 시작된다.
- 예시)
```java
public class Movie {  
    private String title;  
...
```
- 제목, 상영시간, 기본요금 등을 인스턴스 변수로 포함
- 추가적으로, 할인 조건 목록이나 금액 할인 정책에 사용되는 할인 금액, 비율 할인 정책에 사용되는 할인 비율이 Movie 안에서 직접 정의된다.


'MovieType' 클래스 데이터 중심 설계
- 할인 정책은 영화별로 오직 하나만 지정할 수 잇다.
- 할인 정책의 종류를 결정하는 것은 'MovieType'이다.
- 예시)
```java
...
...(생략)
```
- 메서드를 이용해 내부 데이터를 캡슐화했다.
- 캡슐화를 적용하는 가장 간단한 방법은 접근자(getter)나 수정자(setter)를 추가하는 것이다.


'DiscountConditionType' 데이터 중심 설계
```java
...
```
- 할인 조건 타입을 결정하는 클래스
- 할인 조건 타입에 필요한 데이터를 설정


'DiscountCondition' 데이터 중심 설계
```java
...
```
- 할인 조건 구현에 필요한 데이터를 추가
- 위 클래스는 캡슐화를 깨므로 접근자와 수정자를 추가한다.
