...
- 디미터 법칙은 결합도와 관련되며, 결합도가 문제가 되는 것은 객체의 내부 구조가 외부로 노출되는 경우로 한정된다.
- 여러 개의 도트를 사용한 코드가 문제가 되는지 알기 위해서는 코드가 객체 내부 구조를 노출하고 있는지 여부로 판단한다.

결합도와 응집도의 충돌
- 일반적으로 어떤 객체의 상태를 물어본 후 변환된 상태를 기반으로 결정을 내리고 그 결정에 따라 객체의 상태를 변경하는 코드는 '묻지 말고 시켜라' 스타일로 변경해야 한다.
- 예시
```java
public class Theater {
	public void enter(Audience audience) {
		if (audience.getBag().hasInvitation()) {
			...
			audience.getBag().minusAmount(ticket.getFee());
			ticketSeller.getTicketOffice().plusAmount(ticket.getFee());
			audience.getBag().setTicktet(ticket);
		}
	}
}
```
- Theater는 Audience 내부에 포함된 Bag에 대해 질문한 후 반환된 결과를 이용해 Bag의 상태를 변경
- 이 코드는 Audience의 캡슐화를 위반하기 때문에 Theater는 Audience의 내부 구조에 강하게 결합됨
- 이 문제를 해결하는 방법은 질문하고, 판단하고, 상태를 변경하는 모든 코드를 Audience로 옮기는 것
- 다시 말해 Audience에게 위임 메서드를 추가하는 것
```java
public class Audience {
	public Long buy(Ticket ticket) {
		if(bag.hasInvitation()) {
...
```
- Audience는 상태와 함께 상태를 조작하는 행동을 포함해 응집도가 높아졌다.
- 위임 메서드를 통해 객체 내부 구조를 감추는 것은 협력에 참여하는 객체들의 결합도를 낮출 수 있는 동시에 객체의 응집도를 높일 수 있는 효과적인 방법
- Q 그렇다면 위임 메서드와 디미터 법칙은 반드시 사용해야 좋은가?
	- 모든 상황에 맹목적으로 위임 메서드를 추가하면 같은 퍼블릭 인터페이스 안에 어울리지 않는 오퍼레이션 들이 공존하게 됨
	- 결과적으로 객체는 상관 없는 책임들을 한꺼번에 떠안아 응집도가 낮아짐


명령-쿼리 분리 원칙
 - 루틴: 어떤 절차를 묶어 호출 가능하도록 이름을 부여한 기능 모듈
 - 루틴은 프로시저와 함수로 구분할 수 있다.
 - 프로시저: 정해진 절차에 따라 내부 상태를 변경하는 루틴의 한 종류, 값을 반환할 수 없다.
 - 함수: 어떤 절차에 따라 필요한 값을 계산해서 반환하는 루틴의 한 종류, 부수효과를 발생시킬 수 없다.
...
- 문장으로 요약하면 '질문이 답변을 수정해서는 안 된다'는 것
...
프로시저 추상화
- 기능 분해(functional decomposition)와 관련
- 알고리즘 분해(algorithmic decompostion)라고 부르기도 한다.

데이터 추상화
- 타입을 추상화하는 추상 데이터 타입(abstract data type)
- 프로시저를 추상화하는 객체지향


프로시저 추상화와 기능 분해
- 기능은 오랜 시간 시스템을 분해하기 위한 기준으로 사용
- 이 같은 시스템 분해 방식을 '알고리즘 분해' 또는 '기능 분해'라고 부른다.
- 기능 분해의 관점에서 추상화의 단위는 프로시저이며 시스템은 프로시저 단위로 분해

프로시저
- 반복적으로 실행되거나 거의 유사하게 실행되는 작업들을 하나의 장소에 모아놓음으로써 로직을 재사용하고 중복을 방지할 수 있는 추상화 방법
- 프로시저를 추상화라고 부르는 이유는 내부의 상세한 구현을 모르더라도 인터페이스만 알면 프로시저를 사용할 수 있기 때문

전통적인 기능 분해 방법
- 하향식 접근법(Top-Down)을 따름
...
- 기본적으로 기능 분해는 책의 목차를 정리하고 그 안에 내용을 채워 넣은 것과 유사
- 기능 분해 방법에서는 기능을 중심으로 필요한 데이터를 결정

하향식 기능 분해의 문제점
- 시스템은 하나의 메인함수로 구성돼 있지 않다.
- 기능 추가나 요구사항 변경으로 인해 메인 함수를 빈번하게 수정해야 하나다.
...
하나의 메인 함수라는 비현실적인 아이디어
 ...
 - 모든 기능들은 규모라는 측면에서 차이가 있을 수 있지만 기능성의 측면에서는 동등하게 독립적이고 완결된 하나의 기능을 표현한다.
 - 하향식 접근법은 하나의 알고리즘을 구현하거나 배치 처리를 구현하기에 적합하지만 현대적인 상호작용 시스템을 개발하는 데 적합하지 않다.
