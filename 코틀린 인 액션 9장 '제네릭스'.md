...
- 예제:
```kotlin
interface Produce<in T> {
	fun produce(value: T)
}

class FruitProducer : Producer<Fruit> {
	override fun produce(value: Fruit) {
...
fun main() {
	val fruitProducer: Producer<Apple> = FruitProducer()

	// Producer<Apple>은 Producer<Fruit>의 하위 타입이므로 할당이 가능
	val producerOfFruit: Producer<Fruit> = fruitProducer

	producerOfFruit.produce(Apple())
	producerOfFruit.produce(Banana())
}
```
...
- 이후에 `producerOfFruit`를 통해 `produce` 메서드를 호출할 때, `Apple`과 `Banana`를 넘길 수 있습니다. 이는 반공변성으로 인해 `Producer<Apple>`이 `Producer<Fruit>`의 하위 타입이 되어 하위 타입 관계가 유지되기 때문
- 
